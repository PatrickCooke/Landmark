...
-(void)annotateMapLocations {
    ...
    NSLog(@"Adding %li pins",_landmarkArray.count);
    for (Landmarks *currentlandmark in _landmarkArray) {
...
//    for (int i =0; (i <= 10); i++) {
//        NSIndexPath *indexPath = i;
//        Landmarks *currentlandmark = _landmarkArray[indexPath.row];
//        MKPointAnnotation *lm1 = [[MKPointAnnotation alloc]init];
//        lm1.coordinate = CLLocationCoordinate2DMake(@"%f, %f", currentlandmark.landmarkLat, currentlandmark.landmarkLon);
//        lm1.title = currentlandmark.landmarkName;
//        lm1.subtitle = currentlandmark.landmarkAddress;
//
//    }
...
- (void)viewDidLoad {
    ...
    //[self tempAddRecords];
    _landmarkArray = [[NSArray alloc] init];
    _landmarkArray= [ self fetchLandmarks];
    NSLog(@"total records %li",_landmarkArray.count);
